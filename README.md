...
- локальной защиты доступа (PIN/биометрия).
...
- Отдельный экран `Groups`: создание/применение/дублирование профилей групп + Join по QR.
- Quick Start onboarding: `Connect -> Groups -> Test message`.
- Панель `Link Health`: `loss`, `latency`, `retry rate`, `queue depth`.
- App Lock: PIN/биометрия, ручная и авто-блокировка.
...
## Quick Start (онбординг)

После первого успешного подключения открывается мастер:
1. `Connect` — проверка подключения.
2. `Groups` — создание группы или вход по QR.
3. `Test message` — отправка тестового сообщения в чат.

Мастер можно завершить (`FINISH`) или пропустить (`SKIP`).
...
Откройте вкладку `Groups` (в header/mobile tab bar), затем:
1. Заполните:
   - название группы;
   - канал;
   - общий E2EE ключ.
2. Нажмите `CREATE + APPLY`.
...
- `JOIN BY QR` / `SCAN INVITE QR` — сканировать QR и автоматически вступить в группу (канал + ключ применяются сразу).
...
В верхней панели приложения отображается `LINK HEALTH`:
- `loss` — доля failed среди завершенных исходящих;
- `latency` — медиана времени от `sent` до `ack/delivered/read`;
- `retry rate` — доля сообщений с повторными попытками;
- `queue depth` — глубина очереди outbox.

Если состояние `bad` держится непрерывно, появляется баннер `LINK DEGRADED`.

## Защита доступа (PIN / биометрия)

Раздел `Groups -> SECURITY`:
- `ENABLE PIN LOCK` — включить PIN (4-8 цифр);
- `UPDATE PIN` — сменить PIN;
- `ENABLE BIOMETRY` — включить биометрию (если поддерживается WebAuthn);
- `DISABLE LOCK` / `DISABLE BIOMETRY` — отключить защиту.

Поведение:
- при включенном lock приложение открывается через `SecurityGate`;
- есть ручная блокировка кнопкой `🔒` в header;
- есть авто-блокировка при уходе приложения в background.
...
│   │   ├── security.ts         # PIN/biometric helpers
│   │   └── demo.ts
│   ├── App.tsx
│   ├── main.tsx
│   └── index.css
